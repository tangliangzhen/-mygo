...
from legged_gym.envs.GO2_Stand.GO2_Duijiaostand.Go2_duijiaostand_Config import GO2Cfg_Duijiaostand,GO2CfgPPO_Duijiaostand
def get_euler_xyz_tensor(quat):
    ...
    return euler_xyz
class Go2_duijiaostand(BaseTask):
    def __init__(self, cfg: GO2Cfg_Duijiaostand, sim_params, physics_engine, sim_device, headless):
    ...
    def _reward_lin_vel_z(self):
        ...
        return torch.exp(-torch.abs(self.base_lin_vel[:, 2])*10)
    
    def _reward_ang_vel_xy(self):
        # Penalize xy axes base angular velocity
        return torch.exp(-torch.norm(torch.abs(self.base_ang_vel[:, 0:2]), dim=1))
    ...
    def _reward_tracking_lin_vel(self):
        x_error = torch.square(self.commands[:, 0] - self.base_lin_vel[:, 0])#站立起来的话，本体的x轴对应世界系的z，本体z轴对应世界系的-x
    ...
    def _reward_default_pos_reward(self):
        ...
        # 修正1：取所有12个关节（去掉[:,:6]的6，改为全维度）
        joint_diff = torch.abs(self.dof_pos - self.descire_joint_pos)
        
        # 修正2：区分腿型权重（强化RL/FR收缩、FL/RR接地）
        # FL(0-2) + RR(9-11)：接地腿，权重1.0（严格要求接近目标）
        fl_rr_diff = joint_diff[:, 0:3] + joint_diff[:, 9:12]
        # RL(3-5) + FR(6-8)：收缩腿，权重0.5（更容易拿奖励，鼓励收缩）
        rl_fr_diff = 0.5 * (joint_diff[:, 3:6] + joint_diff[:, 6:9])
        
        total_diff = torch.sum(fl_rr_diff + rl_fr_diff, dim=1)
        return torch.exp(-total_diff) * (torch.mean(self.rew_hanstand) > 0.70)
    ...
    def _reward_duijiaostand_feet_on_air(self):
        """
        脚部在空奖励：
        ...
        # return reward
        # # return 0
            # contact_forces: (num_envs, num_bodies, 3)，取RL（1）、FR（2）的接触力
        rl_fr_contact = torch.norm(self.contact_forces[:, [self.feet_indices[1], self.feet_indices[2]], :], dim=-1) > 0.5
        # RL/FR都离地则奖励1，否则奖励0
        reward = torch.where(
            (~rl_fr_contact).all(dim=1),  # 所有维度都未触地（等价于原prod(dim=1)）
            torch.tensor(1.0, device=self.device),  # 离地奖励1
            torch.tensor(-10.0, device=self.device)  # 触地惩罚（负数）
        )
        return reward


    def _reward_duijiaostand_orientation(self):
        """
        姿态奖励：
    ...
    def _reward_default_hip_pos(self):
        ...
        # return joint_diff
        fl_rr_hip_diff = torch.abs(self.dof_pos[:,0]) + torch.abs(self.dof_pos[:,9])
        return fl_rr_hip_diff
    ...
    def _reward_feet_clearance(self):#鼓励抬脚高度
        ...
        left_feet_height=self.feet_height[:,1]
        right_feet_height=self.feet_height[:,2]
        # Compute swing mask
        ...
        rew=torch.exp(-torch.abs(left_feet_height-target_height)*10)
        # print(rew[0],torch.sum(torch.abs(left_feet_height-target_height),dim=1)[0])
        rew+=torch.exp(-torch.abs(right_feet_height-target_height)*10)
    ...
    def _reward_symmetric_joints(self):
        ...
        dof[:,3,0] *= -1
            ...
            # 奖励FL-RL对称、FR-RR对称（仅惩罚不对称）
        err_fl_rl = torch.sum(torch.abs(dof[:,0,:] - dof[:,1,:]), axis=1)
        err_fr_rr = torch.sum(torch.abs(dof[:,2,:] - dof[:,3,:]), axis=1)
        err = err_fl_rl + err_fr_rr
        return err*(self.rew_hanstand>0.70)
        # err = torch.sum(torch.abs(dof[:,0,:] - dof[:,1,:]),axis=1) #+ torch.sum(torch.abs(dof[:,2,:] - dof[:,3,:]),axis=1)
        # return err*(self.rew_hanstand>0.70)

    
    def _reward_duijiaostand_feet_height_exp(self):
