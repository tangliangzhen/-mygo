from legged_gym.envs.GO2_Stand.GO2_Duijiaostand.Go2_duijiaostand_Config import GO2Cfg_Duijiaostand
import math
import numpy as np
...
def on_press(key):
    ...
    try:
        ...
        # 核心添加：裁剪三个控制量到[-1.8, 1.8]区间
        # np.clip(变量, 最小值, 最大值)：超出范围则强制截断到边界值
        x_vel_cmd = np.clip(x_vel_cmd, -1.8, 1.8)
        y_vel_cmd = np.clip(y_vel_cmd, -1.8, 1.8)
        yaw_vel_cmd = np.clip(yaw_vel_cmd, -1.8, 1.8)   
...
def run_mujoco(policy, cfg):
    ...
    # model.opt.density = np.random.uniform(0.9, 1.1) * model.opt.density
    # model.opt.viscosity = np.random.uniform(0.9, 1.1) * model.opt.viscosity
    # for geom in range(model.ngeom):
    #     model.geom_friction[geom] = np.random.uniform(0.2, 0.8)  # 
    ...
    for _ in range(int(cfg.sim_config.sim_duration / cfg.sim_config.dt)):
        ...
        if count_lowlevel % cfg.sim_config.decimation == 0:
            ...
            #target_q = action * cfg.control.action_scale - 0.75 * cfg.robot_config.default_dof_pos
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_model', type=str, default="logs/go2_duijiaostand/exported/policies/policy_1.pt",help='Run to load from.')
    parser.add_argument('--terrain', action='store_true', help='terrain or plane')
    args = parser.parse_args()

    class Sim2simCfg(GO2Cfg_Duijiaostand):
        class sim_config:
