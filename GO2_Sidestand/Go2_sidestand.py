...
from legged_gym.envs.GO2_Stand.GO2_Sidestand.Go2_sidestand_Config import GO2Cfg_Sidestand,GO2CfgPPO_Sidestand
def get_euler_xyz_tensor(quat):
    ...
    return euler_xyz
class Go2_sidestand(BaseTask):
    def __init__(self, cfg: GO2Cfg_Sidestand, sim_params, physics_engine, sim_device, headless):
    ...
    def post_physics_step(self):
        ...
        self.feet_pos = self.rigid_state.view(self.num_envs, self.num_bodies, 13)[:, self.feet_name_reward_indices, 0:3]
        self.feet_contact_pos = self.rigid_state.view(self.num_envs, self.num_bodies, 13)[:, self.contact_foot_indices, 0:3]
        self.feet_contact_vel = self.rigid_state.view(self.num_envs, self.num_bodies, 13)[:, self.contact_foot_indices, 7:10]
    ...
    def compute_observations(self):
        ...
        domain_randomization_info = torch.cat((
                    self.env_frictions,
                    self.added_base_masses,        # 1
                    self.added_base_com,          # 3
                    self.p_gains_multiplier ,             # num_actions
                    self.d_gains_multiplier,             # num_actions
                    self.joint_armatures,         # 1
                    self.joint_friction_coeffs,        # 1
                    self.joint_damping_coeffs,          # 1
                    self.restitution_coeffs,
                    self.restitution_coeffs,
            ), dim=-1)
        
        self.privileged_obs_buf = torch.cat(
                                    (self.base_lin_vel * self.obs_scales.lin_vel,self.obs_buf,domain_randomization_info,contact_mask)
                                    ,dim=-1)
    def create_sim(self):
    ...
    def _process_rigid_body_props(self, props, env_id):
        ...
        if self.cfg.domain_rand.randomize_base_mass:
            self.added_base_masses[env_id, 0] = torch_rand_float(self.cfg.domain_rand.added_base_mass_range[0], self.cfg.domain_rand.added_base_mass_range[1], (1, 1), device=self.device)
            props[0].mass += self.added_base_masses[env_id, 0]
        ...
        if self.cfg.domain_rand.randomize_base_com:
            self.added_base_com[env_id,:] = torch_rand_float(self.cfg.domain_rand.added_base_com_range[0], self.cfg.domain_rand.added_base_com_range[1], (1, 3), device=self.device)
            props[0].com += gymapi.Vec3(self.added_base_com[env_id, 0], self.added_base_com[env_id, 1],
                                    self.added_base_com[env_id, 2])
        return props
    ...
    def _resample_commands(self, env_ids):
        ...
        all_zero_mask = torch.rand(len(env_ids), device=self.device) < 0.20
        self.commands[env_ids[all_zero_mask]] = 0.0

        # 4. 新增：另外 5% 仅前两项（x,y 线速度）归零
        xy_zero_mask = torch.rand(len(env_ids), device=self.device) >0.9
    ...
    def _compute_torques(self, actions):
        ...
        """
        actions = actions * self.cfg.control.action_scale
        # pd controller
        p_gains = self.p_gains * self.p_gains_multiplier
        d_gains = self.d_gains * self.d_gains_multiplier

        act=(actions + self.motor_zero_offsets)#.unsqueeze(1).repeat(1,12)#*(self.episode_length_buf>50)
    ...
    def _reset_root_states(self, env_ids):
        ...
        """
        # base position
        if self.custom_origins:#平地就不是，随机地形就要随机位置
    ...
    def _create_envs(self):
        ...
        feet_name_reward=[]
        for name in self.cfg.asset.feet_name_reward:
            feet_name_reward.extend([s for s in body_names if name in s])
        
        contact_foot=[]
        for name in self.cfg.asset.contact_foot:
            contact_foot.extend([s for s in body_names if name in s])
        ...
        self.feet_name_reward_indices = torch.zeros(len(feet_name_reward), dtype=torch.long, device=self.device, requires_grad=False)
        for i in range(len(feet_name_reward)):
            self.feet_name_reward_indices[i] = self.gym.find_actor_rigid_body_handle(self.envs[0], self.actor_handles[0], feet_name_reward[i])
    ...
    def _reward_base_height(self):
        ...
        self.rew_hanstand=torch.mean(torch.exp(-torch.abs(base_height - self.cfg.rewards.base_height_target)*5))
    ...
    def _reward_tracking_lin_vel(self):
        x_error = torch.square(self.commands[:, 0] - self.base_lin_vel[:, 0])#站立起来的话，本体的z轴对应世界系的y，本体y轴对应世界系的-z
        y_error = torch.square(self.commands[:, 1] - self.base_lin_vel[:, 2])
    ...
    def _reward_default_pos(self):
        ...
        # return torch.sum(torch.abs(self.dof_pos - self.descire_joint_pos), dim=1)
            ...
            # 左腿关节索引：FL(0,1,2)、RL(3,4,5)；右腿(6,7,8,9,10,11)不惩罚
        left_joint_indices = [0,1,2,3,4,5]
        # 仅计算左腿关节与默认位置的偏差
        
        right_joint_indices = [6, 7, 8, 9, 10, 11]
        # 对右腿偏离默认位置的惩罚非常轻，权重为0.1，给它足够的活动空间
        right_joint_diff = torch.sum(torch.abs(self.dof_pos[:, right_joint_indices] - self.descire_joint_pos[right_joint_indices]), dim=1) * 0.1
        left_joint_diff = torch.sum(torch.abs(self.dof_pos[:, left_joint_indices] - self.descire_joint_pos[left_joint_indices]), dim=1)
        return left_joint_diff + right_joint_diff
        
    def _reward_default_pos_reward(self):
        # # Penalize motion at zero commands
        # return torch.exp(-torch.sum(torch.abs(self.dof_pos - self.descire_joint_pos)[:,:6], dim=1))*(torch.mean(self.rew_hanstand)>0.70)
            # 仅计算左腿关节偏差
        left_joint_indices = [0,1,2,3,4,5]
        left_joint_diff = torch.sum(torch.abs(self.dof_pos[:, left_joint_indices] - self.descire_joint_pos[left_joint_indices]), dim=1)
        # 偏差越小奖励越高，且仅当基座高度达标时生效
        return torch.exp(-left_joint_diff*5)*(torch.mean(self.rew_hanstand)>0.70)
    ...
    def _reward_sidestand_feet_on_air(self):
        """
        脚部在空奖励：
        ...
        # reward = (~contact).float().prod(dim=1)
         # 关键修改：不触地→奖励1；触地→惩罚-5（而非原有的0）
        reward = torch.where(
        ~torch.any(contact, dim=1),  # 所有右后腿都没触地
        torch.tensor(1.0, device=self.device),  # 奖励
        torch.tensor(-5.0, device=self.device)   # 触地重罚
    )
    ...
    def _reward_sidestand_orientation(self):
        """
        姿态奖励：
    ...
    def _reward_feet_clearance(self):#鼓励抬脚高度
        ...
        # left_feet_height=self.feet_height[:,2]
        # right_feet_height=self.feet_height[:,3]
        # # Compute swing mask
        ...
        #     # 仅计算右腿（FR_foot=2, RR_foot=3）离地高度
        # self.feet_height = self.rigid_state[:, self.feet_indices, 2] - 0.02
        # right_feet_height = self.feet_height[:, 2:4]  # FR、RR高度
        
        # # 目标高度：保持0.06m离地（不触地），允许±0.02微调
        # target_height = torch.ones_like(right_feet_height) * self.cfg.rewards.target_foot_height
        # # 惩罚高度偏离目标值，偏差越小奖励越高
        # rew = torch.exp(-torch.abs(right_feet_height - target_height)*20).sum(dim=1)
        
        # # 仅当基座高度达标时生效
        # return rew*(torch.mean(self.rew_hanstand)>0.70)
        
        self.feet_height = self.rigid_state[:, self.feet_indices, 2] - 0.02
        right_feet_height = self.feet_height[:, 2:4]  # 2=右前腿(FR), 3=右后腿(RR)
        
        # 目标高度：右前腿保持现有高度，右后腿目标抬高（强制抬起）
        target_height = torch.ones_like(right_feet_height) * self.cfg.rewards.target_foot_height
        target_height[:, 1] = 0.15  # 右后腿的目标离地高度→比右前腿高（强迫抬起）
        
        # 计算高度误差：给右后腿的误差加2倍权重（策略更关注它）
        height_error = torch.abs(right_feet_height - target_height)
        height_error[:, 1] *= 2  # 右后腿误差权重×2→抬不起来惩罚更重
        
        # 确保右后腿至少抬到基础高度（低于0.05直接重罚）
        height_error[:, 1] += torch.where(right_feet_height[:, 1] < 0.15, 10.0, 0.0)
        
            # --- 新增：惩罚抬腿速度过快 ---
        # 获取右腿关节的速度
        right_joint_vel = self.dof_vel[:, 6:12]  # FR(6-8), RR(9-11)
        # 如果关节速度超过阈值（例如 5 rad/s），则施加惩罚
        vel_penalty = torch.sum((torch.abs(right_joint_vel) - 5.0).clip(min=0.0), dim=1) * 0.1
        # ------------------------------------
        ...
        # 偏差越小奖励越高
        rew = torch.exp(-height_error*20).sum(dim=1) - vel_penalty 
        return rew*(torch.mean(self.rew_hanstand)>0.70)
    ...
    def _reward_contact(self):
        ...
        # contact_foot_indices是FL_foot、RL_foot，必须都接触（sum=2）
        contact = self.contact_forces[:, self.contact_foot_indices, 2] > 5
        # 仅当左前+左后腿都接地且基座高度达标时奖励
        return (torch.sum(contact,dim=1)==2)*(self.rew_hanstand>0.70)
    ...
    def _reward_feet_air_time(self):
        ...
        # # rew_airTime *= torch.norm(self.commands[:, :2], dim=1) 
        # self.feet_air_time *= ~contact_filt
        # return rew_airTime*(self.rew_hanstand>0.70)
        
            # 仅计算右腿（FR、RR）空中时间
        right_foot_indices = self.feet_name_reward_indices  # FR、RR的索引
        contact = self.contact_forces[:, right_foot_indices, 2] > 1.
        contact_filt = torch.logical_or(contact, self.last_contacts[:, :2])  # 仅右腿
        self.last_contacts[:, :2] = contact
        first_contact = (self.feet_air_time[:, :2] > 0.) * contact_filt
        self.feet_air_time[:, :2] += self.dt
        
        #rew_airTime = torch.sum((self.feet_air_time[:, :2] - 0.1) * first_contact, dim=1)
        # 关键修改：右后腿（索引1）的空中时间奖励×2，持续悬空奖励更高
        rew_airTime = (self.feet_air_time[:, 0] - 0.1) * first_contact[:, 0]
        rew_airTime += (self.feet_air_time[:, 1] - 0.1) * first_contact[:, 1] * 2.0  # 右后腿额外奖励
       

        
        self.feet_air_time[:, :2] *= ~contact_filt
        # 仅当基座高度达标且左腿接地时生效
        return rew_airTime*(self.rew_hanstand>0.70)*(torch.sum(self.contact_forces[:, self.contact_foot_indices, 2]>1, dim=1)==2)
    ...
    def _reward_sidestand_feet_height_exp(self):
        # feet_indices_tensor = torch.tensor(feet_indices, dtype=torch.long, device=self.rigid_body_pos.device)
        feet_height_error = torch.abs(self.feet_pos[:,:,2]- 0.67).sum(dim=1)
        # print(torch.exp(-feet_height_error / (std**2)))
        # print(feet_height_error.shape)
        return torch.exp(-feet_height_error *10)
        # return 0
